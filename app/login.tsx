...
export default function LoginScreen() {
  ...
  const fieldStyle = { minHeight: 48, backgroundColor: colors.mutedCard, borderRadius: 8, borderWidth: 1, borderColor: colors.border, paddingLeft: 44, paddingRight: 13, paddingVertical: 12, color: colors.text, fontSize: 16 } as const;
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: colors.page }}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 22 }}>
    <View style={{ width: Math.min(440, Math.max(0, viewport.width - 44)), alignSelf: 'center', gap: 24 }}>
      <View style={{ gap: 8, alignItems: 'center' }}><View style={{ width: 64, height: 64, borderRadius: 8, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', shadowColor: colors.primary, shadowOpacity: Platform.OS === 'ios' ? 0.2 : 0.12, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 3 }}><Server color="#fff" size={31} strokeWidth={2.1} /></View><Text style={{ color: colors.text, fontSize: 34, fontWeight: '800', marginTop: 5 }}>Lucky</Text><Text style={{ color: colors.subtext, fontSize: 15 }}>管理控制台</Text></View>
      <View style={{ gap: 15, padding: 18, borderRadius: 8, backgroundColor: colors.card, borderWidth: Platform.OS === 'ios' ? 0 : 1, borderColor: colors.border, shadowColor: colors.shadow, shadowOpacity: Platform.OS === 'ios' || Platform.OS === 'web' ? 0.07 : 0, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: Platform.OS === 'android' ? 1 : 0 }}>
        ...
        <Pressable onPress={submit} disabled={isSubmitting} style={({ pressed }) => ({ backgroundColor: isSubmitting ? colors.disabled : colors.primary, borderRadius: 8, minHeight: 50, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, opacity: pressed ? 0.72 : 1 })}>{isSubmitting ? <ActivityIndicator color="#fff" /> : <LogIn color="#fff" size={19} />}<Text style={{ color: '#fff', fontWeight: '700', fontSize: 16 }}>{isSubmitting ? '正在登录' : '登录'}</Text></Pressable>
      </View>
    </View>
