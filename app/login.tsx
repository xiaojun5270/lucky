...
import { Eye, EyeOff, KeyRound, Link2, LockKeyhole, LogIn, UserRound } from 'lucide-react-native';
...
const schema = z.object({
  ...
  twoFACode: z.string().refine((value) => !value || /^\d{6}$/.test(value), '请输入6位数字验证码'),
});
...
export default function LoginScreen() {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { baseUrl: luckySessionState.baseUrl, account: luckySessionState.account, password: luckySessionState.password, twoFACode: '' } });
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: colors.page }}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 22 }}>
    <View style={{ width: Math.min(440, Math.max(0, viewport.width - 44)), alignSelf: 'center', gap: 24 }}>
      <View style={{ gap: 8, alignItems: 'center' }}><View style={{ width: 82, height: 82, alignItems: 'center', justifyContent: 'center', shadowColor: colors.shadow, shadowOpacity: Platform.OS === 'ios' ? 0.14 : 0.1, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 3 }}><ExpoImage source={require('../assets/logo.png')} contentFit="contain" style={{ width: 82, height: 82 }} /></View><Text style={{ color: colors.text, fontSize: 34, fontWeight: '800', marginTop: 5 }}>Lucky</Text><Text style={{ color: colors.subtext, fontSize: 15 }}>管理控制台</Text></View>
      <View style={{ gap: 16, padding: 20, borderRadius: 20, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, shadowColor: colors.shadow, shadowOpacity: Platform.OS === 'ios' || Platform.OS === 'web' ? 0.05 : 0, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: Platform.OS === 'android' ? 1 : 0 }}>
        ...
        <View style={{ gap: 7 }}><Text style={{ color: colors.text, fontSize: 13, fontWeight: '600' }}>2FA 验证码（可选）</Text><View><KeyRound color={colors.subtext} size={18} style={{ position: 'absolute', left: 14, top: 14, zIndex: 1 }} /><Controller name="twoFACode" control={control} render={({ field }) => <TextInput {...field} onChangeText={(value) => field.onChange(value.replace(/\D/g, '').slice(0, 6))} placeholder="6位动态验证码" placeholderTextColor={colors.placeholder} keyboardType="number-pad" autoComplete="one-time-code" textContentType="oneTimeCode" maxLength={6} style={fieldStyle} />} /></View>{errors.twoFACode ? <Text style={{ color: colors.danger, fontSize: 12 }}>{errors.twoFACode.message}</Text> : null}</View>
