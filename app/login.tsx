...
export default function LoginScreen() {
  ...
  const fieldStyle = { backgroundColor: colors.mutedCard, borderRadius: 8, paddingLeft: 44, paddingRight: 13, paddingVertical: 13, color: colors.text, fontSize: 15 } as const;
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: colors.page }}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 22 }}>
    <View style={{ width: Math.min(460, Math.max(0, viewport.width - 44)), alignSelf: 'center', gap: 22 }}>
      <View style={{ gap: 10 }}><View style={{ width: 54, height: 54, borderRadius: 8, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', shadowColor: colors.primary, shadowOpacity: 0.22, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 3 }}><Server color="#fff" size={27} /></View><Text style={{ color: colors.text, fontSize: 30, fontWeight: '800' }}>Lucky 管理</Text><Text style={{ color: colors.subtext, lineHeight: 21 }}>连接你的 Lucky 2.27.2 服务。凭据仅保存在当前设备的安全存储中。</Text></View>
