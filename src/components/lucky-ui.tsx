...
export function MetricCard({ icon, label, value, color, background, valueColor }: { icon: LucideIcon; label: string; value: string; color: string; background: string; valueColor?: string }) {
  const colors = useAppTheme();
  return <View style={{ flexGrow: 1, flexBasis: 150, minHeight: 124, borderRadius: CARD_RADIUS, paddingHorizontal: 14, paddingTop: 14, paddingBottom: 15, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, ...surfaceShadow(Platform.OS) }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}><IconTile icon={icon} color={color} background={background} size={34} iconSize={17} /><Text numberOfLines={1} style={{ flex: 1, color: colors.subtext, fontSize: 12, lineHeight: 17, fontWeight: '600' }}>{label}</Text></View><Text numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.78} style={{ marginTop: 'auto', color: valueColor ?? colors.text, fontSize: 19, lineHeight: 25, fontWeight: '800' }}>{value || '--'}</Text></View>;
}
